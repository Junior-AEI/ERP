...
describe('ROUTE (GET): /api/projectNote (Get all projectNotes)', () => {
    ...
    it('Normal usage', async () => {
        ...
        const res = await request(app).get('/api/projectNote').set('Authorization', `Bearer ${token}`)
...
describe('ROUTE (GET): /api/projectNote/:projectNoteId (Get a specific project note)', () => {
    ...
    it('Wrong format id', async () => {
        ...
        for (const wrongFormatId of wrongFormatIdList) {
            const res = await request(app).get(`/api/projectNote/${wrongFormatId}`).set('Authorization', `Bearer ${token}`)
    ...
    it('Wrong noteId with a good format', async () => {
        ...
        const res = await request(app).get(`/api/projectNote/${-10}`).set('Authorization', `Bearer ${token}`)
    ...
    it('Normal usage', async () => {
        ...
        const res = await request(app).get(`/api/projectNote/${note?.noteId}`).set('Authorization', `Bearer ${token}`)
...
describe('ROUTE (DELETE): /api/projectNote/:id (Delete project note)', () => {
    ...
    it('Wrong format noteId', async () => {
        ...
        for (const wrongParam of wrongParamList) {
            const res = await request(app).delete(`/api/projectNote/${wrongParam}`).set('Authorization', `Bearer ${token}`)
            expect(res.status).toEqual(400)
        }
    })
    ...
    it('Wrong noteId (note not found)', async () => {
        ...
        const res = await request(app).delete(`/api/projectNote/${-10}`).set('Authorization', `Bearer ${token}`)
        expect(res.status).toEqual(404)
    })
    ...
    it('Good usage', async () => {
        ...
        const res = await request(app).delete(`/api/projectNote/${noteId}`).set('Authorization', `Bearer ${token}`)
