...
describe('ROUTE (GET): /api/person (Get all persons)', () => {
    ...
    it('Normal usage', async () => {
        ...
        const res = await request(app).get('/api/person').set('Authorization', `Bearer ${token}`)
...
describe('ROUTE (GET): /api/person/:personId (Get a specific person)', () => {
    ...
    it('Wrong format', async () => {
        ...
        const wrongFormatPersonIdList = [null, undefined, 'wrongParamId']

        for (const wrongFormatPersonId of wrongFormatPersonIdList) {
            const res = await request(app).get(`/api/person/${wrongFormatPersonId}`).set('Authorization', `Bearer ${token}`)
    ...
    it('Normal usage', async () => {
        ...
        const res = await request(app).get(`/api/person/${person?.personId}`).set('Authorization', `Bearer ${token}`)
...
describe('ROUTE (POST): /api/person (Create new person)', () => {
    ...
    it('Wrong firstname', async () => {
        ...
        const wrongParamList = ['', null, undefined, 'firstnameToooooooooooooooooooooooooooooooooLong']
    ...
    it('Wrong lastname', async () => {
        ...
        const wrongParamList = ['', null, undefined, 'lastnameToooooooooooooooooooooooooooooooooLong']
    ...
    it('Wrong mobilePhone', async () => {
        ...
        const wrongParamList = ['', null, undefined, 'numero', 10000, 1234567890123456]
...
describe('ROUTE (PUT): /api/person/:id (Update person)', () => {
    ...
    it('Wrong firstname', async () => {
        ...
        const wrongParamList = ['', null, undefined, 'firstnameToooooooooooooooooooooooooooooooooLong', 19]
    ...
    it('Wrong lastname', async () => {
        ...
        const wrongParamList = ['', null, undefined, 'lastnameToooooooooooooooooooooooooooooooooLong', 19]
...
describe('ROUTE (DELETE): /api/person/:id (Delete person)', () => {
    ...
    it('Wrong format personId', async () => {
        ...
        const wrongParamList = [null, undefined, 'wrongPersonIdFormat']

        for (const wrongParam of wrongParamList) {
            const res = await request(app).delete(`/api/person/${wrongParam}`).set('Authorization', `Bearer ${token}`)
            expect(res.status).toEqual(400)
        }
    })
    ...
    it('Wrong personId (person not found)', async () => {
        ...
        const res = await request(app).delete(`/api/person/${-10}`).set('Authorization', `Bearer ${token}`)
        expect(res.status).toEqual(404)
    })
    ...
    it('Good usage', async () => {
        ...
        const res = await request(app).delete(`/api/person/${personId}`).set('Authorization', `Bearer ${token}`)
